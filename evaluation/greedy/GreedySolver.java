...
// The main class that abstracts a greedy SCP solver
	public abstract class GreedySolver<T> {
	protected String _name;			  // name of algorithm type
	protected double _alpha;          // minimum required coverage level
	protected SCPModel<T> _model;        // the SCP model we're currently operating on
	protected TreeSet<ElementSet<T>> _solnSets; // could use array instead
	SortedSet<T> _elementsNotCovered; // set of the elements currently not covered by solution
	...
	public void setModel(SCPModel<T> model) { _model = model; }
	...
	public void reset() {
		_solnSets = new TreeSet<ElementSet<T>>();
	...
	public void solve() {
		...
		_elementsNotCovered = new TreeSet<T>();
		...
		while (_elementsNotCovered.size() > 0 && 
			...
			ElementSet<T> nextBestSet = this.nextBestSet();
	...
	public abstract ElementSet<T> nextBestSet();
